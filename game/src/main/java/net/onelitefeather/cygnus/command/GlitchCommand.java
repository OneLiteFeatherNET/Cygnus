...
public final class GlitchCommand extends Command {
    ...
    /** Completes the sentence {@code CommandSenders} sends back to a sender without a screen. */
    ...
    public GlitchCommand(SlenderGazeService service) {
        ...
        this.addSyntax((sender, context) -> {
            Player player = CommandSenders.asPlayer(sender, Messages.ONLY_PLAYERS_HAVE_A_VIEW);
            if (player == null) return;
            service.preview(player, context.get(level) - 1);
        }, level);

        this.addSyntax((sender, context) -> {
            Player player = CommandSenders.asPlayer(sender, Messages.ONLY_PLAYERS_HAVE_A_VIEW);
            if (player == null) return;
            service.preview(player, SlenderGaze.NONE);
