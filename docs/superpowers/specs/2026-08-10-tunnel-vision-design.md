...
pack/assets/cygnus/font/overlay.json
```
...
at stage 16, where a plain ellipse reads as a tunnel rather than a frame. `U+E000`–`U+E00F` map to
stages 1–16 in `cygnus:overlay`, the font shared with the blood splatter — see
[the blood splatter design](2026-08-11-blood-splatter-design.md) for why they live together.
Both are generated by `tools/generate_overlay.py` in `cygnus-pack`.
...
The renderer hands its glyph to the shared `ScreenOverlay`, which owns the title and composes the
layers — the blood splatter uses the same channel and either one sending its own title would wipe
the other. Three details that otherwise look broken:
...
- `OverlayTunnelVisionRenderer` — the implementation described above; it contributes a glyph to
  the shared `ScreenOverlay` rather than sending a title itself.
...
| Stage drops to 0 | the layer is dropped rather than drawn — otherwise the last vignette lingers until the title's `stay` runs out |
...
- `OverlayTunnelVisionRendererTest` — Cyano: the renderer contributes the expected code point in
  the `cygnus:overlay` font, the shadow is transparent, and `clear()` drops only its own layer
  rather than wiping the screen out from under the blood splatter.
